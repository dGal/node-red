                ...
                if (node.wires) {
                    ...
                    for (j=0;j<node.wires.length;j++) {
                        ...
                        for (k=0;k<wires.length;k++) {
                            var _wid = (wires[k] && typeof wires[k] === 'object') ? wires[k].id : wires[k];
                            linkMap[node.id].push(_wid);
                            nn = oldConfig.allNodes[_wid];
                            if (nn) {
        ...
        if (newConfig.allNodes.hasOwnProperty(id)) {
            ...
            if (node.wires) {
                ...
                for (j=0;j<node.wires.length;j++) {
                    wires = node.wires[j];
                    for (k=0;k<wires.length;k++) {
                        var _widN = (wires[k] && typeof wires[k] === 'object') ? wires[k].id : wires[k];
                        if (linkMap[node.id].indexOf(_widN) === -1) {
                            linkMap[node.id].push(_widN);
                        }
                        nn = newConfig.allNodes[_widN];
                        if (nn) {
                            linkMap[nn.id] = linkMap[nn.id] || [];
