...
function handlePreDeliver(flow,sendEvent, reportError) {
    ...
    hooks.trigger("preDeliver",sendEvent,(err) => {
        if (err) {
            ...
            // Attach inbound input metadata before delivery (index and optional label)
            try {
                if (sendEvent && sendEvent.destination && (typeof sendEvent.destination.input === 'number')) {
                    const idx = sendEvent.destination.input;
                    let label;
                    const destNode = sendEvent.destination.node;
                    let cfg = null;
                    if (flow && flow.flow && flow.flow.nodes && destNode) {
                        cfg = flow.flow.nodes[destNode.id];
                    }
                    if (!cfg && flow && flow.global && flow.global.allNodes && destNode) {
                        cfg = flow.global.allNodes[destNode.id];
                    }
                    if (cfg && cfg.inputLabels) {
                        if (Array.isArray(cfg.inputLabels)) {
                            label = cfg.inputLabels[idx];
                        } else if (typeof cfg.inputLabels === 'string') {
                            label = cfg.inputLabels;
                        }
                    }
                    sendEvent.msg._input = { index: idx };
                    if (label !== undefined) {
                        sendEvent.msg._input.label = label;
                    }
                }
            } catch(e) {}
