...
RED.nodes = (function() {
    ...
    function addLink(l) {
        if (nodeLinks[l.source.id]) {
            const isUnique = nodeLinks[l.source.id].out.every(function(link) {
                return link.sourcePort !== l.sourcePort ||
                       link.target.id !== l.target.id ||
                       ((link.targetPort||0) !== (l.targetPort||0))
            })
            if (!isUnique) {
                return
            }
    ...
    function getLinkId(link) {
        return link.source.id + ':' + link.sourcePort + ':' + link.target.id + ':' + ((link.targetPort!=null)?link.targetPort:0)
    }
            ...
            for (var j=0;j<wires.length;j++) {
                ...
                if (w.target.type != "subflow") {
                    if (w.sourcePort < node.wires.length) {
                        var targetEntry = (w.targetPort != null) ? { id: w.target.id, port: (w.targetPort || 0) } : w.target.id;
                        node.wires[w.sourcePort].push(targetEntry);
                    }
                }
            }

            if (n.inputs > 0 && n.inputLabels)  {
    ...
    function convertSubflow(n, opts) {
        ...
        if (node.in.length > 0 && n.inputLabels)  {
            node.inputLabels = n.inputLabels.slice();
        }
            ...
            if (n.wires) {
                for (var w1=0;w1<n.wires.length;w1++) {
                    ...
                    for (var w2=0;w2<wires.length;w2++) {
                        var wEntry = wires[w2];
                        var wid = (wEntry && typeof wEntry === 'object') ? wEntry.id : wEntry;
                        var tPort = (wEntry && typeof wEntry === 'object' && wEntry.port != null) ? wEntry.port : 0;
                        if (node_map.hasOwnProperty(wid)) {
                            if (n.z === node_map[wid].z) {
                                var link = {source:n, sourcePort:w1, target:node_map[wid], targetPort:tPort};
                                addLink(link);
                                new_links.push(link);
                            } else {
                                console.log("Warning: dropping link that crosses tabs:", n.id, "->", node_map[wid].id);
                            }
                        }
                    }
                }
    ...
    function filterLinks(filter) {
        ...
        for (var n=0;n<candidateLinks.length;n++) {
            ...
            if (filter.hasOwnProperty("targetPort") && ((link.targetPort||0) !== filter.targetPort)) {
                continue;
            }
            result.push(link);
        }
