...
RED.editor = (function() {
    ...
    function updateNodeProperties(node, outputMap) {
        ...
        // Allow any non-negative number of inputs (multi-input support)
        node.inputs = Math.max(0, parseInt(node.inputs));
        if (isNaN(node.inputs)) {
            node.inputs = 0;
        }
        // Remove links if inputs reduced to 0 or reduced below existing targetPort indices
        if (node.inputs === 0) {
            removedLinks = removedLinks.concat(RED.nodes.filterLinks({target:node}));
        } else if (node.hasOwnProperty("__inputs")) {
            if (node.inputs < node.__inputs) {
                var toRemove = RED.nodes.filterLinks({target:node}).filter(function(l){ return (l.targetPort||0) >= node.inputs; });
                // Ensure uniqueness
                toRemove.forEach(function(l){
                    if (removedLinks.indexOf(l) === -1) { removedLinks.push(l) }
                })
            }
            delete node.__inputs;
        }
