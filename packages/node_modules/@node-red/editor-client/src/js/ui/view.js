...
RED.view = (function() {
    ...
    function showQuickAddDialog(options) {
        ...
        var rebuildQuickAddLink = function() {
            ...
            var numPorts = (quickAddLink.portType === PORT_TYPE_OUTPUT) ? (quickAddLink.node.outputs || 1) : (quickAddLink.node.inputs || 1);
            var sourcePort = quickAddLink.port;
            var portY = -((numPorts-1)/2)*13 + 13*sourcePort;
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                ...
                var numPorts = (drag_line.portType === PORT_TYPE_OUTPUT) ? (drag_line.node.outputs || 1) : (drag_line.node.inputs || 1);
    ...
    function spliceLink(link, node, historyEvent) {
        ...
        const link2 = {
            ...
            sourcePort: 0,
            target: link.target,
            targetPort: (link.targetPort != null) ? link.targetPort : 0
        };
    ...
    function updateSelection() {
        ...
        var selectionJSON = activeWorkspace+":"+JSON.stringify(selection,function(key,value) {
            if (key === 'nodes' || key === 'flows') {
                ...
                return value.source.id+":"+value.sourcePort+":"+value.target.id+":"+(value.targetPort||0);
            } else if (key === 'links') {
                return value.map(function(link) {
                    return link.source.id+":"+link.sourcePort+":"+link.target.id+":"+(link.targetPort||0);
                });
            }
            return value;
        });
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode == RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                if (portType != drag_lines[i].portType && mouseup_node !== drag_lines[i].node) {
                    ...
                    var link = {source: src, sourcePort:src_port, target: dst};
                    // record targetPort for multi-input rendering, if available
                    if (drag_line.portType === PORT_TYPE_OUTPUT) {
                        link.targetPort = portIndex || 0;
                    } else if (drag_line.portType === PORT_TYPE_INPUT) {
                        link.targetPort = drag_line.port || 0;
                    }
                    if (drag_line.virtualLink) {
                        ...
                        if (!(
                            ...
                            var tPort = (drag_line.portType === PORT_TYPE_OUTPUT) ? (portIndex || 0) : (drag_line.port || 0);
                            var existingLink = RED.nodes.filterLinks({source:src,target:dst,sourcePort: src_port, targetPort: tPort}).length !== 0;
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    var labelParts;
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs || this.__inputs__.length !== (d.inputs||0)) {
                        labelParts = getLabelParts(label, "red-ui-flow-node-label");
                        if (labelParts.lines.length !== this.__labelLineCount__ || this.__label__ !== label) {
                            d.resize = true;
                        }
                        ...
                        var portRows = Math.max((d.outputs || 0), (d.inputs || 0));
                        if (hideLabel) {
                            d.h = Math.max(node_height, portRows * 15);
                        } else {
                            d.h = Math.max(6+24*labelParts.lines.length, portRows * 15, 30);
                        }
                        this.__hideLabel__ = hideLabel;
                    }
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        // Manage input ports (support multiple)
                        var numInputs = d.inputs || 0;
                        // For link in nodes, only show virtual input port when linking between link nodes
                        if (d.type === "link in") {
                            if ((isLink && (showAllLinkPorts===PORT_TYPE_INPUT || activeLinkNodes[d.id]))) {
                                numInputs = 1;
                            } else {
                                numInputs = 0;
                            }
                        ...
                        // Remove extra input port groups
                        while (this.__inputs__.length > numInputs) {
                            var iport = this.__inputs__.pop();
                            if (!d.__ghost) {
                        ...
                        // Add missing input port groups
                        for (var inIdx = this.__inputs__.length; inIdx < numInputs; inIdx++) {
                            var inGroup = document.createElementNS("http://www.w3.org/2000/svg","g");
                            inGroup.setAttribute("class","red-ui-flow-port-input");
                            var inPortEl;
                            if (d.type === "link in") {
                                inPortEl = document.createElementNS("http://www.w3.org/2000/svg","circle");
                                inPortEl.setAttribute("cx",-1);
                                inPortEl.setAttribute("cy",5);
                                inPortEl.setAttribute("r",5);
                                inPortEl.setAttribute("class","red-ui-flow-port red-ui-flow-link-port");
                            } else {
                                inPortEl = document.createElementNS("http://www.w3.org/2000/svg","rect");
                                inPortEl.setAttribute("rx",3);
                                inPortEl.setAttribute("ry",3);
                                inPortEl.setAttribute("width",10);
                                inPortEl.setAttribute("height",10);
                                inPortEl.setAttribute("class","red-ui-flow-port");
                            }
                            inGroup.appendChild(inPortEl);
                            inGroup.__port__ = inPortEl;
                            inPortEl.__data__ = this.__data__;
                            inPortEl.__portType__ = PORT_TYPE_INPUT;
                            inPortEl.__portIndex__ = inIdx;

                            if (!d.__ghost) {
                                inPortEl.addEventListener("mousedown", portMouseDownProxy);
                                inPortEl.addEventListener("touchstart", portTouchStartProxy);
                                inPortEl.addEventListener("mouseup", portMouseUpProxy);
                                inPortEl.addEventListener("touchend", portTouchEndProxy);
                                inPortEl.addEventListener("mouseover", portMouseOverProxy);
                                inPortEl.addEventListener("mouseout", portMouseOutProxy);
                                RED.hooks.trigger("viewAddPort",{node:d,el: this, port: inGroup, portType: "input", portIndex: inIdx})
                            } else if (d.__ghostClick) {
                                inPortEl.addEventListener("mousedown", d.__ghostClick);
                                inPortEl.addEventListener("touchstart", d.__ghostClick);
                            }

                            this.appendChild(inGroup);
                            this.__inputs__.push(inGroup);
                        }
                        ...
                        var numInputs = (d.type === "link in" && (showAllLinkPorts===PORT_TYPE_INPUT||activeLinkNodes[d.id])) ? 1 : (d.inputs || 0);
                        var iy = (d.h/2)-((numInputs-1)/2)*13;
                        for (var ip=0; ip<this.__inputs__.length; ip++) {
                            var ig = this.__inputs__[ip];
                            ig.setAttribute("transform","translate(-5,"+((iy + 13*ip)-5)+")");
                        }
            ...
            var link = linkLayer.selectAll(".red-ui-flow-link").data(
                linksToDraw,
                function(d) {
                    return d.source.id+":"+d.sourcePort+":"+d.target.id+":"+(d.targetPort || d.target.i || 0);
                }
            );
            ...
            link.each(function(d) {
                ...
                if (d.added || d.selected || dirtyNodes[d.source.id] || dirtyNodes[d.target.id]) {
                    ...
                    var sy = -((numOutputs-1)/2)*13 + 13*sourcePort;
                    var numInputs = d.target.inputs || 1;
                    var targetPort = d.targetPort || 0;
                    var ty = -((numInputs-1)/2)*13 + 13*targetPort;
    ...
    function calculateNodeDimensions(node) {
        ...
        var labelParts = getLabelParts(label, "red-ui-flow-node-label");
        var portRows = Math.max((node.outputs || 0), (node.inputs || 0));
        if (hideLabel) {
            result[1] = Math.max(node_height, portRows * 15);
        } else {
            result[1] = Math.max(6+24*labelParts.lines.length, portRows * 15, 30);
        }
        if (hideLabel) {
